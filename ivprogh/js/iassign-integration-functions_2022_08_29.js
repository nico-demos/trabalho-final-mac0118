...
// 2022/08/28 iLM_file, iLM_n
var iLMparameters = {
  iLM_file: getParameterByName("iLM_file"),
  iLM_n   : getParameterByName("iLM_n"),
...
console.log("iassign-integration-functions.js: iLMparameters.iLM_PARAM_AssignmentURL=" +
  iLMparameters.iLM_PARAM_AssignmentURL + ", iLM_PARAM_Assignment=" + iLMparameters.iLM_PARAM_Assignment);
...
// This function is called by iAssign to get iVProg content (in student evaluation/send or teacher authoring)
// The return is a JSON with the student answer to the exercise or its code
// This content will be registered in Moodle DataBase by iAssign
function getAnswer () {
  console.log("iassign-integration-functions.js: getAnswer(): iLMparameters.iLM_PARAM_activityType=" +
               iLMparameters.iLM_PARAM_activityType + ", iLM_PARAM_SendAnswer=" + iLMparameters.iLM_PARAM_SendAnswer); //2021 //DEP
  ...
  //R Verificar:
  //R * 'iLM_PARAM_SendAnswer == true' ou vazio => deve ser enviado para servidor
  //R * 'iLM_PARAM_SendAnswer == false'         => NAO deve ser enviado para servidor
  //R Se o parametro "iLM_PARAM_SendAnswer" for false, entao trata-se de resolucao de atividade
  if (iLMparameters.iLM_PARAM_SendAnswer == 'false') {
    // Montar o retorno com a resposta do aluno
    var contentToSend = "";
    if (previousContent) { //2021: "test"
      contentToSend = previousContent.split("\n::algorithm::")[0];
      } //2021: "test"
    else
      contentToSend = "{\n \"testcases\" : [\n ]\n}"; // buildTestCases(); //
    contentToSend += '\n::algorithm::\n';
...
function getiLMContent () {
  ...
  $.get(iLMparameters.iLM_PARAM_Assignment, function (data) {
    //professor invocou a avaliacao automatica dos exercicios do bloco
...
function prepareEnvironment () {
  ...
  //2022/08/28: just in case 'iLMparameters = {...}' do not initialize correctly
  console.log("0: iLMparameters.iLM_PARAM_Assignment=" + iLMparameters.iLM_PARAM_Assignment + ", iLM_file=" + iLMparameters.iLM_file + ", iLM_n=" + iLMparameters.iLM_n); //DEP
  if (iLMparameters.iLM_file == "up") // move one directory up: iLMparameters.iLM_PARAM_Assignment = '../' + iLMparameters.iLM_PARAM_Assignment
    iLMparameters.iLM_PARAM_Assignment = '../' + iLMparameters.iLM_PARAM_Assignment;
  if (iLMparameters.iLM_PARAM_Assignment==null || iLMparameters.iLM_PARAM_Assignment==undefined) {
    iLMparameters.iLM_PARAM_Assignment = getParameterByName("iLM_PARAM_Assignment");
    iLMparameters.iLM_file = getParameterByName("iLM_file");
    }

  //2021/10/09: //DEP
  console.log("1: iassign-integration-functions.js: prepareEnvironment(): iLMparameters.iLM_PARAM_activityType=" + iLMparameters.iLM_PARAM_activityType); //DEP
  console.log("2: iLMparameters.iLM_PARAM_Assignment=" + iLMparameters.iLM_PARAM_Assignment + ", iLM_file=" + iLMparameters.iLM_file + " : " + window.location.search); //DEP

  //2021/10/09:
  if (!iLMparameters.iLM_PARAM_Assignment || iLMparameters.iLM_PARAM_Assignment.length < 4) { // it is not URL
    renderAlgorithm();
    return;
    } // empty

  //TODO: Rever essa logica 'iLM_PARAM_SendAnswer', pois e' o contrario
  //TODO: Se 'iLM_PARAM_SendAnswer' inexistente, vazio ou 'true' entao NAO enviar resposta do usuario ao servidor
  //TODO: Senao, em qq outra situacao, enviar!
  if (iLMparameters.iLM_PARAM_SendAnswer == 'false') {
    // If iLM_PARAM_SendAnswer=="false", then it is activity resolution, so resolution "DIV" must be enabled/un-hiden
    //$('.resolucao').css("display","block");
    if (iLMparameters.iLM_PARAM_Assignment!=-1 && iLMparameters.iLM_PARAM_activityType!=2)
      getiLMContent();
    ...
    // Whenever it is not "activity resolution" (role "student"), nor "activity view" (role "teacher"), then is "activity authoring"
    //$('.elaboracao').css("display","block");

    // If parameter iLMparameters.iLM_PARAM_Assignment is set to an IVPH URL file, it is example/test/exercise to be loaded.
    // If parameter iLMparameters.iLM_PARAM_activityType = {1,2,3} it is, respectively, example, test or exercise
    getiLMContent();
  ...
  console.log("4: prepareEnvironment(): final"); //DEP 2021
